...
@RestController("staffWritingTestController")
@RequestMapping("/staff/writing-tests")
...
public class WritingTestController {
    ...
    public ResponseEntity<?> getAllTests(@RequestParam(required = false) String name,
        ...
        return ResponseEntity.ok(testService.getAllTestsByType(ETestType.WRITING, name, status, page, size, sortBy, direction));
    }

    @GetMapping("/{id}")
    public ResponseEntity<?> getTestById(@PathVariable Long id) {
        return ResponseEntity.ok(testService.getWritingTestDetailById(id));
    }
