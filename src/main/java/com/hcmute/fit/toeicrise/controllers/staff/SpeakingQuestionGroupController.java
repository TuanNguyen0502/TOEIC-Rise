...
@RestController
@RequestMapping("/staff/speaking-question-groups")
@RequiredArgsConstructor
public class SpeakingQuestionGroupController {
    ...
    public ResponseEntity<?> updateQuestionGroup(@PathVariable Long id, @Valid @ModelAttribute SWQuestionGroupUpdateRequest request) {
        return ResponseEntity.ok(questionGroupService.updateSpeakingQuestionGroup(id, request));
    }

    @GetMapping("/{id}")
    public ResponseEntity<?> getQuestionGroup(@PathVariable Long id) {
        return ResponseEntity.ok(questionGroupService.getSpeakingQuestionGroupResponse(id));
    }
}
